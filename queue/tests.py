...
class TestQueueEmpty(unittest.TestCase):
    ...
    def test_empty_dequeue(self):
        len_queue = self.s_queue.size()
        self.assertEqual(self.s_queue._dequeue_status, 0)
        self.assertIsNone(self.s_queue.dequeue())
        self.assertEqual(self.s_queue._dequeue_status, 2)
...
class TestQueueManyItems(unittest.TestCase):
    ...
    def test_many_items_dequeue(self):
        len_queue = self.s_queue.size()
        self.assertEqual(self.s_queue._dequeue_status, 0)
        self.assertEqual(self.s_queue.dequeue(), self.items_list[0])
        self.assertEqual(self.s_queue._dequeue_status, 1)
        self.assertEqual(self.s_queue.size(), len_queue - 1)
