...
class Character(ABC, Generic[T]):
    ...
    # постусловие: получение урона от противника на n-единиц
    @abstractmethod
    def damage(self, n: int) -> None: ...

    # предусловие: значение показателя здоровья меньше 100%
    # постусловие: восстановлено здоровье на n-единиц
    @abstractmethod
    ...
    @abstractmethod
    def get_recuperate_status(self) -> int: ...  # успешно;
    # показатель меньше нуля;
...
class AbstractEnemyFactory(ABC, Generic[T]):
    ...
    @abstractmethod
    def build_enemy(self) -> AbstractEnemy: ...
